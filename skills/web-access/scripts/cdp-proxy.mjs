...
// 要求：Chrome 已允许当前实例的 remote debugging（见 chrome://inspect/#remote-debugging）
// Node.js 22+ 默认使用原生 WebSocket
...
if (typeof globalThis.WebSocket !== 'undefined') {
  ...
  try {
    ...
    console.error('[CDP Proxy] 错误：Node.js 版本 < 22，且当前运行环境无法解析 ws 模块');
    console.error('  解决方案：升级到 Node.js 22+，或在当前 Node 运行环境中提供可解析的 ws 依赖');
    process.exit(1);
  }
}
...
async function connect() {
  ...
  if (!chromePort) {
    ...
    if (!discovered) {
      throw new Error(
        'Chrome 未开启或未允许当前实例的 remote debugging。请先在 Chrome 打开：\n' +
        '  chrome://inspect/#remote-debugging\n' +
        '并勾选 "Allow remote debugging for this browser instance"。\n' +
        '如果你是手动启动调试端口，也可使用 --remote-debugging-port=9222。'
      );
    }
