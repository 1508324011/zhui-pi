...
const GLYPHS = [...'曰遂古之初谁传道上下未形何由考冥昭瞢暗能极冯翼惟象识明时三合本化圜则九重孰营度兹功作斡维焉系天加八柱当东南亏际安放属隅隈多有数所沓十二分日月列星陈出汤谷次蒙汜自昏晦行几里夜光德死育利顾菟腹女歧无取伯强处惠气阖辟角宿旦曜灵藏任鸿师尚佥忧课鸱龟曳衔听顺欲帝刑永遏羽山施禹愎变纂绪续业谋洪泉深窴方坟河海应尽历康回怒故倾错洿溢修椭衍昆仑悬圃增城扉启烛照羲扬华冬夏石林兽虬龙雄虺首儵忽不长生守蓱衢枲蛇黑玄趾延寿鲮鬿羿彃乌解献省土嵞台桑嗜快晁饱代益卒离蠥拘达射鞫害躬播降棘宾辩歌勤屠竟地夷孽民雒嫔珧决封豨蒸膏浞纯狐眩交吞揆'];
const BG: RGB = [22, 8, 43]; // outrun night
const TEXT: RGB = [243, 238, 255];
const CANDY: readonly RGB[] = [
  [255, 42, 109], // neon pink
  [255, 106, 158], // neon pink light
  [217, 119, 255], // neon violet
  [167, 139, 250], // electric violet
  [0, 240, 255], // electric cyan
  [110, 242, 200], // neon mint
];
...
function workingIndicatorFrame(): string {
  return colorize(WORKING_INDICATOR, CANDY[0] ?? [255, 42, 109], true);
}
...
export function createDrops(width: number, density: number, height: number): Drop[] {
  ...
  return selected.slice(0, 96).map((x, index) => {
    ...
    return {
      ...
      color: CANDY[index % CANDY.length] ?? [255, 42, 109],
    };
  });
}
