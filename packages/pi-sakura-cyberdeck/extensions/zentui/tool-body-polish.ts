...
import { mix, type RGB } from "./gradient";
...
const MINT: RGB = [110, 242, 200];
const LAVENDER: RGB = [167, 139, 250];
const SKY: RGB = [0, 240, 255];
const MUTED: RGB = [169, 155, 174];
const DIM_RGB: RGB = [113, 104, 121];
const SAKURA: RGB = [255, 42, 109];
...
/**
 * Sky → electric violet gradient for tool titles. Softer than the neon-pink
 * sakura gradient — keeps the synthwave chrome without the harsh pink.
 */
function renderTitleGradient(text: string): string {
	const from: RGB = [0, 240, 255]; // electric cyan
	const to: RGB = [167, 139, 250]; // electric violet
	const chars = [...text];
	if (chars.length === 0) return text;
	const span = Math.max(1, chars.length - 1);
	return chars
		.map((char, index) => (char === " " ? char : fg(mix(from, to, index / span), char)))
		.join("");
}
...
function styleToolTitle(plain: string, theme?: ThemeLike): string | null {
	...
	const title = renderTitleGradient(rawName);
	if (!rest) return title;
	// Path in sakura so it matches repeated path lines below (same content, same color).
	return `${title}  ${themeOr(theme, "customMessageLabel", LAVENDER, rest)}`;
}
