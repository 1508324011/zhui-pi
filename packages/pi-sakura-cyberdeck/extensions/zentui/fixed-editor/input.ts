...
/** Global variant for extracting every mouse event from a buffered chunk. */
const SGR_MOUSE_ALL_RE = /\u001b\[<(\d+);(\d+);(\d+)([Mm])/g;
...
export function parseMouseEvent(data: string): MouseEvent | undefined {
	...
	return parseSgrMatch(match);
}

/**
 * Parse every SGR mouse event in a buffered input chunk. Terminals batch
 * mouse-move events, so a single data chunk may contain many drag events;
 * processing only the first one makes dragging feel laggy.
 */
export function parseAllMouseEvents(data: string): MouseEvent[] {
	const events: MouseEvent[] = [];
	SGR_MOUSE_ALL_RE.lastIndex = 0;
	let match: RegExpExecArray | null;
	while ((match = SGR_MOUSE_ALL_RE.exec(data)) !== null) {
		events.push(parseSgrMatch(match));
	}
	return events;
}

function parseSgrMatch(match: RegExpExecArray): MouseEvent {
