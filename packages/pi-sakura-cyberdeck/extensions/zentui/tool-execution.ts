...
const MINT = [110, 242, 200] as const;
...
// Status rail = theme semantic tokens from synthwave.json (pastel, not traffic lights).
...
const RAIL_WORKING: [number, number, number] = [0, 240, 255]; // sky  #00F0FF — in flight
const RAIL_SUCCESS: [number, number, number] = [110, 242, 200]; // mint  #6EF2C8 — theme success
	...
	const cleanupBashRender = installPrototypePatch(
		...
		({ predecessor, receiver, args }) => {
			...
			for (let i = 0; i < lines.length; i++) {
				...
				if (cmd) {
					out.push(`${fg(MINT, "❯")} ${fg([0, 240, 255], cmd[1] ?? "")}`);
					continue;
				}
