...
/**
 * Persisted input history so up/down arrow navigation survives restarts.
 * Pi's editor keeps history in memory only; this mirrors it to disk.
 */
const HISTORY_PATH = join(getAgentDir(), "sakura-cyberdeck-history.json");
const HISTORY_LIMIT = 500;

function loadPersistedHistory(): string[] {
	try {
		if (!existsSync(HISTORY_PATH)) return [];
		const raw = JSON.parse(readFileSync(HISTORY_PATH, "utf8")) as { entries?: unknown };
		if (!Array.isArray(raw.entries)) return [];
		return raw.entries.filter((entry): entry is string => typeof entry === "string");
	} catch {
		return [];
	}
}

function persistHistoryEntry(text: string): void {
	// Bash commands (!xxx) are transient shell invocations, not user input.
	if (text.startsWith("!")) return;
	try {
		const entries = loadPersistedHistory();
		if (entries[0] === text) return; // Dedupe consecutive repeats.
		entries.unshift(text);
		if (entries.length > HISTORY_LIMIT) entries.length = HISTORY_LIMIT;
		writeFileSync(HISTORY_PATH, JSON.stringify({ entries }, null, 1), "utf8");
	} catch {
		// Best-effort persistence; never break the editor over a write failure.
	}
}

/** Inject disk history behind an editor's in-memory history (memory newest first). */
function hydrateEditorHistory(editor: { history?: string[] }): void {
	try {
		const persisted = loadPersistedHistory();
		if (persisted.length === 0 || !Array.isArray(editor.history)) return;
		const existing = new Set(editor.history);
		const fresh = persisted.filter((entry) => !existing.has(entry));
		if (fresh.length > 0) editor.history.push(...fresh);
	} catch {
		// Non-fatal: history navigation simply starts empty.
	}
}
...
function renderPolishedFrame({
	...
	// Pi renders scroll indicators ("─── ↑ N more") in the editor's top/bottom
	// border when content overflows. Keep them instead of replacing with a
	// plain line so the chat box retains its scroll feedback.
	const stripAnsi = (text: string): string => text.replace(/\u001B\[[0-9;]*m/g, "");
	const isScrollIndicator = (line: string): boolean => /more/.test(stripAnsi(line));
	const originalTop = editorFrame[0] ?? "";
	const originalBottom = editorFrame[editorFrame.length - 1] ?? "";
	const top = isScrollIndicator(originalTop)
		? originalTop
		: renderEditorFrameBorder("─".repeat(width), config, uiTheme, colorSource);
	const bottom = isScrollIndicator(originalBottom)
		? originalBottom
		: renderEditorFrameBorder("─".repeat(width), config, uiTheme, colorSource);
...
export class PolishedEditor extends CustomEditor {
	...
	constructor(
		...
		hydrateEditorHistory(this as unknown as { history?: string[] });
	}

	addToHistory(text: string): void {
		super.addToHistory(text);
		persistHistoryEntry(text);
	}
	...
	submitValue(): void {
		super.submitValue();
		// The post-submit empty frame must be painted immediately: a delayed
		// diff frame can otherwise leave the submitted text visible, which also
		// blocks up-arrow history navigation (history only engages on an empty
		// editor).
...
export class WrappedPolishedEditor implements EditorComponent {
	...
	constructor(
		...
		hydrateEditorHistory(base as unknown as { history?: string[] });
	}
