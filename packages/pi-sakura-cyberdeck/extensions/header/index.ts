...
function renderHeader(width: number, availableRows = 0): string[] {
  ...
  const sakura: RGB = [255, 42, 109]; // neon pink
  const peach: RGB = [255, 160, 63]; // sun orange
  const lavender: RGB = [167, 139, 250]; // electric violet
  const sky: RGB = [0, 240, 255]; // electric cyan
  const telemetry = "◈  For the Future  ◈";
  ...
  // Lucy portrait: warm synthwave gradient across the full artwork.
  const from: RGB = [255, 160, 63];
  const to: RGB = [255, 233, 62];
  const art = ANIME_ART.map((line) => {
    const clipped = [...line].slice(0, visibleArtWidth).join("");
    return `${artPad}${gradient(clipped, from, to)}`;
  });
